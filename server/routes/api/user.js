...
router.post('/register', async (req, res) => {
  try {
    ...
    const quickBooksUserData = {
      FullyQualifiedName: name,
      PrimaryEmailAddr: { Address: email },
      PrimaryPhone: { FreeFormNumber: mobile },
      Suffix: name,
      Title: name,
      MiddleName: name,
      Notes: name,
    ...
    try {
      ...
      const userObject = await newUser.save({ session })
      const quickbooksService = new QuickBooksService()
      await quickbooksService.init()
      const quickBooksCustomer = await quickbooksService.createUser(quickBooksUserData)
      userObject.quickbooksID = quickBooksCustomer.Customer.Id
      await userObject.save({ session })
