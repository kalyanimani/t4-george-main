...
onDeleteClick(id){
  ...
  this.props.deleteUser(deleteData)
}
componentWillReceiveProps(nextProps){
 
  if(nextProps.user.deleteuser !== this.props.user.deleteuser){
      Toast.fire({
          type: 'success',
...
render() {
  const {listuser,userloading}=this.props.user;
  var tableResult;
  if(listuser==null ||userloading){
      tableResult=(<tr><td colSpan={4} className="text-center">Loading.....</td></tr>)
  }else{
      if(Object.keys(listuser).length >0){
        var filterData= listuser.filter(createFilter(this.state.searchTerm, KEYS_TO_FILTERS))
          ...
          tableResult=filterData.map(result=>{
                  ...
                  <td>{result.email}</td>
                  <td>{result.mobile}</td>
                  {/* <td><button className="btn btn-link" onClick={()=>this.onEditClick(result)}><span className="kt-badge kt-badge--brand kt-badge--inline kt-badge--pill">Edit</span></button></td>
                  ...
                  <td>
                    {checkPermission(this.props.auth,"USER","UPDATE")&&
                  ...
                  <td>
                  {checkPermission(this.props.auth,"USER","DELETE")&&
        ...
        <div className="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--ver kt-page">
          ...
          <div className="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor kt-wrapper" id="kt_wrapper">
            ...
            <div className="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor">
              {/* begin:: Subheader */}
              <SubHeader first="Home" second="List Users" third=""/>
              {/* end:: Subheader */}
              {/* begin:: Content */}
...
export default connect(mapStateToProps,{listUser,deleteUser})(ListUsers);